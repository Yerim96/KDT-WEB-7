...
function Pt52_answer() {
  const [inputWriter, setInputWriter] = useState("");
  const [inputTitle, setInputTitle] = useState("");
  const [comment, setComment] = useState([]);

  const [inputSearch, setInputSearch] = useState("");
  const [serchType, setSerchType] = useState("writer"); //option 자동값 선택
  ...
  const serchComment = () => {
    ...
    const result = comment.filter((value) => {
      //   console.log(value);
      //   console.log(value[serchType]); //문자열이 되는 순간 배열로 객체에 접근해야한다.
      //   console.log(value[serchType].includes(inputSearch));
